...
# Initialize workspace directories safely
for directory in [ORTHODB_BASE_DIR, ORTHODB_FASTAS_DIR, ORTHODB_CSVS_DIR, ORTHODB_TSVS_DIR]:
...
class FUNGALMENACE:
    ...
    def save_fasta(self, sequences, org=False):
        outpath = ORTHODB_FASTAS_DIR / (self.ortho_file_name if org else f"{self.file_name}.fasta")
        with open(outpath, "w") as f:
            SeqIO.write(sequences, f, "fasta")

    def fetch_query_sequence(self):
        url = f"https://rest.uniprot.org/uniprotkb/{self.uniprot_id}.fasta"
        outpath = ORTHODB_FASTAS_DIR / f"{self.file_name}_query.fasta"
        with open(outpath, "w") as f:
            f.write(requests.get(url).text)
        return outpath

    def run_blastp(self, query_fasta: Path, db_fasta: Path, output_tsv: Path):
        # Convert path objects to strings for compatibility with subshells and blast CLI tools
        os.system(f"makeblastdb -in {str(db_fasta)} -dbtype prot -out {str(ORTHODB_BLAST_DB)}")
        blastp_cline = NcbiblastpCommandline(
            query=str(query_fasta),
            db=str(ORTHODB_BLAST_DB),
            evalue=1e-5,
    ...
    def normalize_species(self, name: str) -> str:
        """Normalize species name by stripping strain identifiers."""
        if pd.isna(name):
            return name
        name = name.strip()
        parts = name.split()
    ...
    def filter_blast_results(self, blast_tsv: Path, query_length, fasta_file: Path):
    ...
    def find_intersection_with_gold(self, gold_csv: Path, filtered_hits):
    ...
    def run(self):
        ...
        out_tsv = ORTHODB_TSVS_DIR / f"blast_results_{self.file_name}.tsv"
        self.run_blastp(query_fasta, ORTHODB_FASTAS_DIR / self.ortho_file_name, out_tsv)

        filtered_hits = self.filter_blast_results(out_tsv, self.query_length, ORTHODB_FASTAS_DIR / self.ortho_file_name)
        gold_df = self.find_intersection_with_gold(DATA_PATH, filtered_hits)
        not_gold_df = filtered_hits[~filtered_hits["organism_name"].isin(gold_df["organism_name"])]
        
        not_gold_path = ORTHODB_CSVS_DIR / f"notgold_similarity_{self.file_name}.csv"
...
def read_notgold_results(folder: Path):
    csvs = sorted(folder.glob("notgold_similarity_*.csv"))
    data, organisms, prots = {}, set(), set()
    for path in csvs:
        df = pd.read_csv(path)
        prot = path.name.split("notgold_similarity_")[1].replace(".csv", "")
...
def generate_excel(data, organisms, all_proteins, output_path: Path):
    ...
    for i, org in enumerate(organisms, start=3):
        ...
        if row_red > 0:
            hdr.fill = RED_FILL; red_rows += 1
        elif row_yellow > 0:
            hdr.fill = YELLOW_FILL; yellow_rows += 1
        else:
            hdr.fill = BLUE_FILL; blue_rows += 1
...
def main():
    # Run ortholog search & alignment pipelines
    for name, uid in uniprot_ids.items():
        FUNGALMENACE(uid, name).run()

    # Aggregate outputs and compile matrix sheet
    data, organisms, prots = read_notgold_results(ORTHODB_CSVS_DIR)
    generate_excel(data, organisms, prots, ORTHODB_EXCEL_PATH)
